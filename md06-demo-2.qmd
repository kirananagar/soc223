...
0 yrs vs 20 yrs are different by 0.083 \* 20, or 1.66 happiness points.
...
The plot shows us the differences as we move up by deciles---since we `factor(inc)`, everything is in reference to the first decile.
...
```{r}
ggpredict(happy5,
          terms = "age", #ggpredict is smart, so it grabs both of the age terms without specifying
          ci_level = NA) |> 
  plot() +
  scale_x_continuous(breaks = seq(from = 15, 
                                  to = 100,
...
With differential mortality, there's a chance that other factors such as health might affect the happiness. There aren't a ton of people who make it past 85, so you have to be careful taking the model's word as gospel.

```{r}
happy6 <- lm(stflife ~ eduyrs + inc + health + age + I(age^3), # I(age^3) adds the cubic term
             data = happy)
get_regression_table(happy6) |> 
  select(term, estimate)
AIC(happy6)
```
...
Interestingly, `happy6` (using a cubic) outperforms the linear model. This makes sense when looking at the plots, because there is not much difference between the plots of `happy5` and `happy6`. I want to try a square root.

```{r}
happy7 <- lm(stflife ~ eduyrs + inc + health + age + I(age^0.5), # I(age^0.5) adds the sqrt term
             data = happy)
get_regression_table(happy7) |> 
  select(term, estimate)
```
